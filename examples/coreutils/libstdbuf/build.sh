...
if [ $USE_MUSLLVM == "true" ];
then
    declare -a bitcode=("libstdbuf.bc" "libc.a.bc" "libc.a")
else
    declare -a bitcode=("libstdbuf.bc" )
fi    
...
MANIFEST=libstdbuf.manifest

if [ $USE_MUSLLVM == "true" ];
then
    cat > ${MANIFEST} <<EOF
{"binary": "libstdbuf_fin",
"native_libs":[],
"name": "libstdbuf",
"static_args": [],
"modules": [],
...
then
    ...
    cp slash/libstdbuf_fin libstdbuf_slashed
else
