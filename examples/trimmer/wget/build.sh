...
if [ $USE_MUSLLVM == "true" ];
then
    declare -a bitcode=("wget.bc" "libc.a.bc" "libc.a")
else
    declare -a bitcode=("wget.bc")
fi    
...
if [ $status -eq 0 ]
then
    cp slash/wget_fin wget_slashed
    strip wget_slashed -o wget_slashed_stripped
else
