...
if [ $USE_MUSLLVM == "true" ];
then
    declare -a bitcode=("objdump.bc" "libc.a.bc" "libc.a")
else
    declare -a bitcode=("objdump.bc")
fi    
...
MANIFEST=objdump.manifest

cat > ${MANIFEST} <<EOF    
{"binary": "objdump_fin", 
"native_libs": [], 
"name": "objdump", 
"args": ["-D","--syms","-s","-w", "name"], 
"modules": [], 
"ldflags": ["-ldl","-O3"], 
"main": "objdump.bc"}
EOF
...
if [ $status -eq 0 ]
then
    cp slash/objdump_fin _objdumpslashed
    strip objdump_slashed -o objdump_slashed_stripped
else
