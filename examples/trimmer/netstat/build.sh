...
if [ $USE_MUSLLVM == "true" ];
then
    declare -a bitcode=("netstat.bc" "libc.a.bc" "libc.a")
else
    declare -a bitcode=("netstat.bc")
fi    
...
if [ $status -eq 0 ]
then
    cp slash/netstat_fin netstat_slashed
    strip netstat_slashed -o netstat_slashed_stripped
else
