...
INTRA_SPEC="onlyonce"
USE_MUSLLVM="false"
OPT_OPTIONS="--disable-inlining --use-pointer-analysis --enable-config-prime"
...
if [ $USE_MUSLLVM == "true" ];
then
    declare -a bitcode=("netperf.bc" "libc.a.bc" "libc.a")
else
    declare -a bitcode=("netperf.bc")
fi    
...
MANIFEST=netperf.manifest

cat > ${MANIFEST} <<EOF    
{"binary": "netperf_fin", 
"native_libs": [], 
"name": "netperf", 
"args": ["-H", "localhost","-t","TCP_RR","-D","5,G","-i","15,1"], 
"modules": [], 
"ldflags": ["-lm","-O3"], 
"main": "netperf.bc"}
EOF
...
if [ $status -eq 0 ]
then
    cp slash/netperf_fin netperf_slashed
    strip netperf_slashed -o netperf_slashed_stripped
else
